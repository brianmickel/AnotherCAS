...
export function test_arg() {
  runIndividualTests([
    ...
    ['arg(-1-i)', '-3/4*pi'],
    ['arg((-1)^(1/3))', '1/3*pi'],
    ['arg(1+exp(i*pi/3))', '1/6*pi'],
    ['arg((-1)^(1/6)*exp(i*pi/6))', '1/3*pi'],
  ]);
  ...
  runSequentialTests([
    ...
    ['arg(-i)', '-1/2*pi'],
    ['arg((a+b*i)/(c+d*i))', 'arctan(b/a)-arctan(d/c)'],
    ['arg(((-1)^(1/2) / (3^(1/2)))^(1/2))', '1/4*pi'],
    ['arg((-1)^(1/6))', '1/6*pi'],
  ]);
}
