...
export const test_dependencies = function() {
  ...
  testResult = findDependenciesInScript(
  ...
  */
  ...
  if (
    testResult.affectsVariables.length === 3 &&
  ...
  if (
    testResult.affectsVariables.length === 1 &&
  ...
  if (testResult.indexOf('AVOID_BINDING_TO_EXTERNAL_SCOPE_VALUE') !== -1) {
    console.log(
      'fail dependency tests 49 found AVOID_BINDING_TO_EXTERNAL_SCOPE_VALUE'
    );
  } else {
    console.log('ok dependency test');
  }
  ...
  if (
    testResult.code === 'PCA = function (M) { return ( cov(M) ); }' &&
  ...
  if (
    testResult.code === 'x = function (a, b) { return ( a[b] ); }' &&
  ...
  if (
    testResult.code === 'x = function (a, b) { return ( dot(a, b) ); }' &&
  ...
  if (
    res.code === 'PCA = function (M) { return ( cov(M) ); }' &&
  ...
  if (
    res.code === 'f = function (x) { return ( 21*x ); }' &&
  ...
  res = computeResultsAndJavaScriptFromAlgebra(
